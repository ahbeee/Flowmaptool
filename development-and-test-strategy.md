...
- Task Table derived from tagged outline nodes, including tag/assignee/due-date filters, editable task fields, readonly tag/category columns, sortable headers, persisted table preferences, column visibility and density controls, and expanded workspace mode.
...
- Outline hierarchy, checklist persistence, Task Table derivation, task field editing, task filtering, task sorting, persisted table preferences, column visibility, density, and expanded task table mode.
