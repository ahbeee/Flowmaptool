...
test('loop back route does not steal clicks from nested tree edges', async () => {
  const mainEntry = join(process.cwd(), 'out', 'main', 'index.js');
  const app = await electron.launch({ args: [mainEntry] });
  const window = await app.firstWindow();
  const createChild = async (parentId: string) => {
  ...
  for (const edgeId of ['e1', 'e3', 'e4']) {
    await clickPathAt(edgeId, 0.5);
    await expect(window.getByTestId(`edge-path-${edgeId}`)).toHaveClass(/edge-path-selected/);
  }

  await clickPathAt('e7', 0.5);
  await expect(window.getByTestId('edge-path-e7')).toHaveClass(/edge-path-selected/);
